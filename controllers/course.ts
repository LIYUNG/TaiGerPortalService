...
// getCourse populates `student_id`, so reads of the student's name/email need a
// course shape whose `student_id` is the populated student rather than an id.
type PopulatedCourse = Omit<ICourse, 'student_id'> & {
  student_id?: IStudent;
};
...
const putMycourses = asyncHandler(async (req, res) => {
  ...
  if (is_TaiGer_Student(user)) {
    ...
    const student = (await StudentService.getStudentByIdWithAgents(
      studentId
    )) as unknown as IStudent | null;

    // Response has already been sent above; if the student is missing just skip
    // the agent-notification side-effect rather than throwing post-response.
    if (!student) {
      logger.info('putMycourses: no student found for agent notification');
      return;
    }

    const agents = (student.agents ?? []) as IAgent[];
    // updateCoursesDataAgentEmail is an asyncHandler-wrapped helper (typed as a
    // 3-arg Express handler) but is actually invoked as a 2-arg (recipient, msg)
    // notifier. Cast to its real call shape — TS-only, no runtime change. See
    // FLAGS in the migration report re: the asyncHandler misuse in services/email.
    const sendAgentEmail = updateCoursesDataAgentEmail as unknown as (
      recipient: { firstname?: string; lastname?: string; address?: string },
      msg: {
        student_id?: string;
...
const downloadJson = asyncHandler(async (req, res) => {
  ...
  if (!analysedJson) {
    logger.error('downloadJson: analysed transcript file not found in S3');
    throw new ErrorResponse(404, 'Analysed transcript not found');
  }
