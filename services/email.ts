...
import { transporter, sendEmail, Recipient } from './email/configuration';
...
// A few reminder templates address the meeting calendar by the recipient's id,
// which is present on the full user documents passed at those call sites but not
// part of the minimal `Recipient` shape.
type RecipientWithId = Recipient & { id: string };

type EmailContent = { subject: string; message: string };

// Shared bilingual scaffold: an "English below" notice, a Chinese greeting +
// body, the split line, then an English greeting + body. Lifting it here removes
// the greeting/split-line boilerplate that was duplicated across ~40 templates.
const bilingual = (recipient: Recipient, zh: string, en: string): string => `\
<p>${ENGLISH_BELOW}</p>
...
// Factory for the common `(recipient, msg) -> { subject, message } -> sendEmail`
// shape. The generic default types the otherwise-untyped payload parameter.
const defineEmail =
  <M = EmailMessagePayload>(
    build: (recipient: Recipient, msg: M) => EmailContent
  ) =>
  async (recipient: Recipient, msg: M) => {
    const { subject, message } = build(recipient, msg);
...
export const updateNotificationEmail = defineEmail((recipient) => ({
  subject:
...
export const updatePermissionNotificationEmail = async (
  recipient: Recipient
) => {
  const subject =
...
export const deleteTemplateSuccessEmail = defineEmail((recipient, msg) => ({
  subject: `Template ${msg.category_name} deleted successfully!`,
  message: bilingual(
    recipient,
...
// TODO
export const sendInvitationReminderEmail = defineEmail((recipient, payload) => {
  const activationLink = queryString.stringifyUrl({
    url: ACCOUNT_ACTIVATION_URL,
    query: { email: recipient.address, token: payload.token }
  });
  return {
    subject: 'TaiGer Portal 開通提醒 / TaiGer Portal Activation Reminder',
    message: bilingual(
      recipient,
...
export const sendInvitationEmail = defineEmail((recipient, payload) => {
  const activationLink = queryString.stringifyUrl({
    url: ACCOUNT_ACTIVATION_URL,
    query: { email: recipient.address, token: payload.token }
  });
  return {
    subject: 'TaiGer Portal 電子信箱驗證 / TaiGer Portal Email verification',
    message: bilingual(
      recipient,
      `<p>請使用以下連結來啟用您的帳戶：</p>
...
export const sendConfirmationEmail = defineEmail<string>((recipient, token) => {
  const activationLink = queryString.stringifyUrl({
    url: ACCOUNT_ACTIVATION_URL,
...
export const sendForgotPasswordEmail = defineEmail<string>(
  (recipient, token) => {
...
export const sendPasswordResetEmail = async (recipient: Recipient) => {
  const subject =
...
export const sendAccountActivationConfirmationEmail = async (
  recipient: Recipient
) => {
...
export const sendAgentUploadedProfileFilesForStudentEmail = defineEmail(
  (recipient, msg) => ({
    subject: `Your ${msg.uploaded_documentname} is successfully uploaded!`,
    // The Chinese greeting here intentionally reads "Hi" (not the usual 嗨),
    // preserved from the original; kept inline rather than via `bilingual`.
    message: `\
<p>${ENGLISH_BELOW}</p>
...
export const sendAgentUploadedVPDForStudentEmail = defineEmail(
  (recipient, msg) => ({
    subject: `您的 ${msg.fileType} 已成功上傳 / Your ${msg.fileType} is successfully uploaded!`,
    message: bilingual(
      recipient,
      `<p>您的顧問 ${msg.agent_firstname} ${msg.agent_lastname} 已幫您上傳 ${msg.uploaded_documentname} </p>
...
export const sendUploadedProfileFilesRemindForAgentEmail = defineEmail(
  (recipient, msg) => {
    const student_name = `${msg.student_firstname} ${msg.student_lastname}`;
    return {
      subject: `New ${msg.uploaded_documentname} uploaded from ${student_name}`,
...
<a href="${BASE_DOCUMENT_FOR_AGENT_URL(
        ...
        `<p>your student ${student_name} has uploaded ${
...
export const sendUploadedVPDRemindForAgentEmail = defineEmail(
  (recipient, msg) => ({
    subject: `新 ${msg.fileType} 上傳從 ${msg.student_firstname} ${msg.student_lastname} / New ${msg.fileType} uploaded from ${msg.student_firstname} ${msg.student_lastname}`,
    // should be for student/agent/editor
    message: bilingual(
      recipient,
...
<p>請至 <a href="${UNI_ASSIST_FOR_AGENT_URL(msg.student_id)}">${
      ...
      `<p>your student ${msg.student_firstname} ${
        msg.student_lastname
      } has uploaded ${msg.uploaded_documentname} </p>
...
export const sendChangedProfileFileStatusEmail = async (
  recipient: Recipient,
...
export const informAgentManagerNewStudentEmail = defineEmail(
  (recipient, payload) => {
    ...
    const agentName = `${payload.agents
      ...
      .join(' ')}`;
    return {
      subject: `新學生 ${studentName} 已被指派給 ${agentName} / New student ${studentName} assigned to ${agentName}`,
      message: bilingual(
        recipient,
...
export const informAgentNewStudentEmail = defineEmail((recipient, msg) => ({
  subject: `新學生 ${msg.std_firstname} ${msg.std_lastname} 已被指派給您 / New student ${msg.std_firstname} ${msg.std_lastname} assigned to you`,
  message: bilingual(
    recipient,
...
export const informStudentTheirAgentEmail = defineEmail((recipient, msg) => {
  let agent;
...
export const informAgentEssayAssignedEmail = defineEmail((recipient, msg) => {
...
export const informAgentStudentAssignedEmail = defineEmail((recipient, msg) => {
  let editors = '';
  ...
  return {
    subject: `Editor assigned for ${msg.std_firstname} ${msg.std_lastname}`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const informEssayWriterNewEssayEmail = defineEmail((recipient, msg) => {
  const thread_url = `${THREAD_URL}/${msg.thread_id}`;
  const docName = msg.program
    ? ` - ${msg.program.school} - ${msg.program.program_name} - ${msg.program.degree} - ${msg.program.semester}`
    : '';
  return {
    subject: `New ${msg.file_type}${docName} assigned to you`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const informEditorNewStudentEmail = defineEmail((recipient, msg) => ({
  subject: `New student ${msg.std_firstname} ${msg.std_lastname} assigned to you`,
  message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const informEditorArchivedStudentEmail = defineEmail(
  (recipient, msg) => ({
    subject: `[Close] Student ${msg.std_firstname} ${msg.std_lastname} is close.`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const informStudentArchivedStudentEmail = defineEmail(
  (recipient, payload) => {
    let agent = '';
    ...
    return {
      subject: `[${recipient.firstname} ${recipient.lastname}] TaiGer Portal service ends`,
      message: `\
<p>嗨 ${recipient.firstname} ${recipient.lastname},</p>
...
export const informStudentTheirEssayWriterEmail = defineEmail(
  (recipient, msg) => {
    const thread_url = `${THREAD_URL}/${msg.thread_id}`;
    const docName = msg.program
      ? ` - ${msg.program.school} - ${msg.program.program_name} - ${msg.program.degree} - ${msg.program.semester}`
...
export const informStudentTheirEditorEmail = defineEmail((recipient, msg) => {
  let editor;
  for (let i = 0; i < msg.editors.length; i += 1) {
    const editor_name = `${msg.editors[i].firstname} ${msg.editors[i].lastname}`;
    if (i === 0) {
...
export const createApplicationToStudentEmail = defineEmail((recipient, msg) => {
  let programList;
...
export const updateCredentialsEmail = defineEmail((recipient) => ({
  subject:
...
export const UpdateStudentApplicationsEmail = async (
  recipient: Recipient,
  msg: EmailMessagePayload
) => {
...
export const NewMLRLEssayTasksEmail = defineEmail((recipient, msg) => {
  let applications_name = '';
  for (let i = 0; i < msg.student_applications.length; i += 1) {
    const program_name = `${msg.student_applications[i].programId.school} ${msg.student_applications[i].programId.program_name}`;
    if (msg.new_app_decided_idx.includes(i)) {
  ...
  return {
    subject: `${msg.sender_firstname} ${msg.sender_lastname} has updated application status and new tasks`,
    // should be for admin/editor/agent/student
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
// For editor, english only
export const NewMLRLEssayTasksEmailFromTaiGer = defineEmail(
  (recipient, msg) => {
    let applications_name = '';
    for (let i = 0; i < msg.student_applications.length; i += 1) {
      const program_name = `${msg.student_applications[i].programId.school} ${msg.student_applications[i].programId.program_name}`;
...
// For editor, agents
export const AdmissionResultInformEmailToTaiGer = defineEmail(
  (recipient, msg) => {
    ...
    return {
      subject: `[${result}] ${student_name} has received ${result} from ${applications_name}`,
      // should be for admin/editor/agent/student
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendNewInterviewMessageInThreadEmail = defineEmail(
  (recipient, msg) => {
    ...
    const student_name = `${msg.student_firstname} - ${msg.student_lastname}`;
    return {
      subject: `[Update] ${msg.writer_firstname} ${msg.writer_lastname} sent a new message > ${interview_name}!`,
      message: bilingual(
        recipient,
...
export const sendNewApplicationMessageInThreadEmail = defineEmail(
  (recipient, msg) => {
    const thread_url = `${THREAD_URL}/${msg.thread_id}`;
    const student_name = `${msg.student_firstname} - ${msg.student_lastname}`;
    const task_name = `${msg.school} ${msg.program_name} ${msg.uploaded_documentname}`;
    return {
      subject: `[Update] ${msg.writer_firstname} ${msg.writer_lastname} sent a new message > ${task_name}!`,
      message: bilingual(
        recipient,
...
export const sendNewGeneraldocMessageInThreadEmail = defineEmail(
  (recipient, msg) => {
    const thread_url = `${THREAD_URL}/${msg.thread_id}`;
    const student_name = `${msg.student_firstname} - ${msg.student_lastname}`;
    return {
      subject: `[Update] ${msg.writer_firstname} ${msg.writer_lastname} provides a new message > ${student_name} ${msg.uploaded_documentname}!`,
      message: bilingual(
        recipient,
...
export const sendSetAsFinalProgramSpecificFileForStudentEmail = async (
  recipient: Recipient,
  msg: EmailMessagePayload
) => {
...
export const sendSetAsFinalProgramSpecificFileForAgentEmail = async (
  recipient: Recipient,
  msg: EmailMessagePayload
) => {
...
export const assignEssayTaskToEditorEmail = defineEmail((recipient, msg) => {
  const THREAD_LINK = new URL(`/document-modification/${msg.thread_id}`, ORIGIN)
    .href;
  return {
    subject: `[TODO] Assign Essay Writer to ${msg.student_firstname} ${msg.student_lastname} ${msg.program_name}`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
// For editor, english only
export const assignDocumentTaskToEditorEmail = defineEmail((recipient, msg) => {
  const THREAD_LINK = new URL(`/document-modification/${msg.thread_id}`, ORIGIN)
    .href;
  return {
    subject: `[New Task] ${msg.student_firstname} ${msg.student_lastname} ${msg.documentname} is assigned to you!`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const assignDocumentTaskToStudentEmail = defineEmail(
  (recipient, msg) => {
    ...
    return {
      subject: `[新任務] ${recipient.firstname} ${recipient.lastname} ${msg.documentname} 指派給你 / [New Task] ${recipient.firstname} ${recipient.lastname} ${msg.documentname} is assigned to you!`,
      // The Chinese greeting intentionally reads "Hi", preserved from the
      // original; kept inline rather than via `bilingual`.
      message: `\
<p>${ENGLISH_BELOW}</p>
...
export const AnalysedCoursesDataStudentEmail = defineEmail(
  (recipient, msg) => ({
    subject: '課程匹配度分析成功 / Course data analysed successfully',
    // should be for admin/editor/agent/student
    message: bilingual(
      recipient,
      `<p>您的的課程匹配度已分析。</p>
...
export const updateCoursesDataAgentEmail = defineEmail((recipient, msg) => {
  const student_name = `${msg.student_firstname} ${msg.student_lastname}`;
  const studentCourseUrl = `${STUDENT_COURSE_URL(msg.student_id)}`;
  return {
    subject: `[TODO] 分析課程 ${student_name} | Course anaylsis for ${student_name}`,
    // should be for admin/editor/agent/student
    message: bilingual(
      recipient,
...
export const sendSomeReminderEmail = async (recipient: Recipient) => {
...
export const sendAssignEditorReminderEmail = defineEmail(
  (recipient, payload) => {
    ...
    return {
      subject: '[DO NOT IGNORE] Assign Editor Reminder',
      // should be for admin/editor/agent/student
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendNoTrainerInterviewRequestsReminderEmail = defineEmail(
  (recipient, payload) => {
...
export const sendAssignTrainerReminderEmail = defineEmail(
  (recipient, payload) => {
    ...
    return {
      subject: `[DO NOT IGNORE] Assign Interview Trainer to ${student_name} - ${program_name}`,
      // should be for admin/editor/agent/student
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendAgentNewMessageReminderEmail = defineEmail(
  (recipient, payload) => {
    const student_name = `${payload.student_firstname} - ${payload.student_lastname}`;
    ...
    return {
      subject: `[New Message] ${student_name}`,
      // should be for admin/editor/agent/student
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendStudentNewMessageReminderEmail = defineEmail(
  (recipient, payload) => {
    const messageUrl = `${STUDENT_COMMUNICATION_THREAD_URL(
      payload.student_id
    )}`;
    return {
      subject: `[New Message] ${recipient.firstname} ${recipient.lastname}`,
      // should be for admin/editor/agent/student
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const MeetingAdjustReminderEmail = async (
...
export const MeetingConfirmationReminderEmail = async (
  recipient: RecipientWithId,
  payload: EmailMessagePayload
) => {
  const taigerUser = `${payload.taiger_user_firstname} ${payload.taiger_user_lastname}`;
...
export const MeetingInvitationEmail = async (
  recipient: Recipient,
  payload: EmailMessagePayload
) => {
...
export const MeetingCancelledReminderEmail = async (
  recipient: Recipient,
  payload: EmailMessagePayload
) => {
  const taigerUser = `${payload.taiger_user.firstname} - ${payload.taiger_user.lastname}`;
...
export const InterviewCancelledReminderEmail = async (
  recipient: Recipient,
  payload: EmailMessagePayload
) => {
  const taigerUser = `${payload.taiger_user.firstname} - ${payload.taiger_user.lastname}`;
...
export const MeetingReminderEmail = defineEmail((recipient, payload) => ({
  subject: `[Meeting Reminder] ${recipient.firstname} ${recipient.lastname}`,
  // should be for admin/editor/agent/student
  message: `\
[會議討論提醒]
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const UnconfirmedMeetingReminderEmail = defineEmail(
  (recipient, payload) => {
    const calendarUrl = `${
      payload.role === 'Student'
    ...
    return {
      subject: `[TODO now] Meeting to confirm reminder ${recipient.firstname} ${recipient.lastname}`,
      // should be for admin/editor/agent/student
      message: `\
[會議時間確認提醒]
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const TicketCreatedAgentEmail = defineEmail((recipient, payload) => {
  ...
  return {
    subject: `[TODO] Update request for ${programName} by ${student_name}`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const TicketResolvedRequesterReminderEmail = defineEmail(
  (recipient, payload) => {
    ...
    return {
      subject: `[Close] Program Update Request for ${programName} by ${taiger_user_name}`,
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const TicketResolvedStudentEmail = defineEmail((recipient, payload) => {
  const programName = `${payload.program.school}-${payload.program.program_name}`;
  const student_name = `${payload.student.firstname} ${payload.student.lastname}`;
  return {
    subject: `[Resolved] Request for ${programName} by ${student_name}`,
    message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendAssignEssayWriterReminderEmail = defineEmail(
  (recipient, payload) => {
    const student_name = `${payload.student_firstname} - ${payload.student_lastname}`;
    return {
      subject: '[DO NOT IGNORE] Assign Essay Writer Reminder',
      // should be for admin/editor/agent/student
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendAssignedInterviewTrainerToTrainerEmail = defineEmail(
  (recipient, payload) => {
    ...
    return {
      subject: `Interview Training Request assigned to you for ${training_request}`,
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendAssignedInterviewTrainerToStudentEmail = defineEmail(
  (recipient, payload) => {
    const program = `${payload.interview.program_id.school} ${payload.interview.program_id.program_name} ${payload.interview.program_id.degree} ${payload.interview.program_id.semester}`;
    const student_name = `${payload.interview.student_id.firstname} ${payload.interview.student_id.lastname}`;
    const training_request = `${student_name} - ${program}`;
    ...
    return {
      subject: `[Info] Interview Trainer assigned for ${training_request}`,
      message: `\
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const InterviewTrainingReminderEmail = defineEmail(
  (recipient, payload) => ({
    subject: `[Training Reminder] ${recipient.firstname} ${recipient.lastname}`,
    // should be for admin/editor/agent/student
    message: `\
[面試訓練討論提醒]
<p>Hi ${recipient.firstname} ${recipient.lastname},</p>
...
export const sendSetAsFinalInterviewEmail = async (
  recipient: Recipient,
  msg: EmailMessagePayload
) => {
...
export const InterviewSurveyRequestEmail = defineEmail((recipient, msg) => {
  ...
  return {
    subject: `[TODO][Urgent] Interview Survey for ${interview_name}`,
    message: bilingual(
      recipient,
...
export const InterviewSurveyFinishedEmail = defineEmail((recipient, msg) => {
  const user_name = `${msg.user.firstname} ${msg.user.lastname}`;
  const student_name = `${msg.interview.student_id.firstname} ${msg.interview.student_id.lastname}`;
  const interviewSurveyUrl = `${SINGLE_INTERVIEW_SURVEY_THREAD_URL(
    msg.interview._id.toString()
  ...
  return {
    subject: `[Close] Interview survey finished: ${interview_name}`,
    message: bilingual(
      recipient,
...
export const InterviewSurveyFinishedToTaiGerEmail = defineEmail(
  (recipient, msg) => {
    const user_name = `${msg.user.firstname} ${msg.user.lastname}`;
    const student_name = `${msg.interview.student_id.firstname} ${msg.interview.student_id.lastname}`;
    const interviewSurveyUrl = `${SINGLE_INTERVIEW_SURVEY_THREAD_URL(
