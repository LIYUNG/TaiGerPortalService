...
describe('GET /api/students/v3/paginated', () => {
  const PAGINATED_URL = '/api/students/v3/paginated';
  ...
  it('returns a page of students with a total count', async () => {
    const resp = await requestWithSupertest
      .get(`${PAGINATED_URL}?page=1&limit=20`)
      .set('tenantId', TENANT_ID);

    expect(resp.status).toBe(200);
    expect(resp.body.success).toBe(true);
    expect(resp.body.data.total).toBe(studentCount);
    expect(resp.body.data.students).toHaveLength(studentCount);
  });

  it('caps the page by limit while total stays the full count', async () => {
    const page1 = await requestWithSupertest
      .get(`${PAGINATED_URL}?page=1&limit=2&sortBy=name_en`)
      .set('tenantId', TENANT_ID);
    const page2 = await requestWithSupertest
      .get(`${PAGINATED_URL}?page=2&limit=2&sortBy=name_en`)
      .set('tenantId', TENANT_ID);

    expect(page1.body.data.students).toHaveLength(2);
    expect(page1.body.data.total).toBe(studentCount);
    expect(page2.body.data.students).toHaveLength(2);
    // Pages are disjoint (stable sort).
    const ids1 = page1.body.data.students.map((s) => s._id);
    const ids2 = page2.body.data.students.map((s) => s._id);
    expect(ids1.filter((id) => ids2.includes(id))).toHaveLength(0);
  });

  it('searches by student name (English)', async () => {
    const resp = await requestWithSupertest
      .get(`${PAGINATED_URL}?search=${encodeURIComponent(student.firstname)}`)
      .set('tenantId', TENANT_ID);

    expect(resp.body.data.total).toBeGreaterThanOrEqual(1);
    const ids = resp.body.data.students.map((s) => s._id);
    expect(ids).toContain(student._id.toString());
    // Every returned student's English name contains the searched term.
    resp.body.data.students.forEach((s) => {
  ...
  it('filters by the name_en column (contains)', async () => {
    const resp = await requestWithSupertest
      .get(`${PAGINATED_URL}?name_en=${encodeURIComponent(student.lastname)}`)
      .set('tenantId', TENANT_ID);
  ...
  it('scopes to students supervised by a given agent id', async () => {
    ...
    const resp = await requestWithSupertest
      .get(`${PAGINATED_URL}?agents=${agent._id}`)
      .set('tenantId', TENANT_ID);
