...
const definitions = [
  ...
  makeTool(
    'get_my_overview',
    'Get a cross-portfolio attention summary for the current user: upcoming AND overdue application deadlines, document threads waiting on the team (with how many days each has stalled), students whose latest message has waited 7+ days for a team reply, students who have gone silent (no message FROM the student in 10+ days), admitted-but-not-confirmed applications, students missing required base documents, and communicationRiskSignals — IMPLICIT risks mined from message CONTENT (frustration, confusion, repeated unanswered questions, broken promises, deadline anxiety, cooling engagement, mentions of competitors/refund, declining sentiment) that the status/time buckets cannot see, each with an evidence quote and how many days it has persisted. IMPORTANT: this returns a small worst-first SAMPLE of students (default 8; hasMoreStudents=true means the list is truncated) — never present it as the complete list; point the user to the AI Assist portfolio view for full triage. Use for "what needs my attention", "who is at risk / has gone silent", and "what hidden/implicit risks am I missing across my students" questions.',
