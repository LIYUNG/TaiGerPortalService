...
const { getAccessibleStudentFilter } = studentAccessModule;
const { buildOverview, PORTFOLIO_BUCKET_LIMIT } = overviewModule;
const { withPostgresRetry } = postgresRetryModule;
...
// helpers read/write; everything is optional since callers pass partial objects.
interface AssistContextShape {
  mentionedStudent?: { id?: string; displayName?: string | null } | null;
  requestedSkill?: string | null;
  unknownSkillText?: string | null;
  analysisMode?: boolean;
}

interface AssistantResultShape {
  activeStudent?: { id?: string; displayName?: string; name?: string } | null;
  skillTrace?: {
...
interface ConversationShape {
  id?: string;
  title?: string | null;
  titleUpdatedByUser?: boolean | null;
  titleAutoGenerated?: boolean | null;
}

interface ConversationUpdateValues {
  updatedAt: Date;
  studentId?: string;
...
// The orchestrator declares a narrower AssistContext (mentionedStudent.displayName
// is string|undefined, not |null). Our resolved payload uses null at runtime, so
// cast through this derived type at the runAiAssist boundary (read-only there).
type OrchestratorAssistContext = NonNullable<
  Parameters<typeof aiAssistOrchestrator.runAiAssist>[1]
>['assistContext'];

interface TitleRefinementPayload {
  conversationId: string;
  ownerUserId: string;
  seedTitle: string;
  message: string;
  assistContext?: AssistContextShape;
  assistantResult: AssistantResultShape;
}
...
const normalizeAutoTitle = (value: unknown) =>
  String(value || '')
    .replace(/[\r\n\t]+/g, ' ')
    .replace(/^["'`]+|["'`]+$/g, '')
    .replace(/\s+/g, ' ')
...
type SkillLabelKey = keyof typeof SKILL_TITLE_LABELS;

const resolveSkillLabel = (
  assistContext: AssistContextShape = {},
  assistantResult: AssistantResultShape = {}
) =>
  SKILL_TITLE_LABELS[assistContext.requestedSkill as SkillLabelKey] ||
  SKILL_TITLE_LABELS[
    assistantResult?.skillTrace?.requestedSkill as SkillLabelKey
  ] ||
  null;

const resolveTitleStudentName = (
  assistContext: AssistContextShape = {},
  assistantResult: AssistantResultShape = {}
) =>
...
const resolveAssistContextPayload = async (
  req: Request
): Promise<AssistContextShape | undefined> => {
...
const resolvePreferredLanguage = (req: Request) => {
...
const isStreamingRequest = (req: Request) => {
...
const buildConversationUpdateValues = ({
  ...
  const values: ConversationUpdateValues = {
    updatedAt: new Date()
  };
...
const queueAiTitleRefinement = ({
  ...
  assistContext,
  assistantResult
}: Partial<TitleRefinementPayload>) => {
  if (
  ...
  if (isStreamingRequest(req)) {
    ...
    try {
      ...
      const assistantResult = await aiAssistOrchestrator.runAiAssist(postgres, {
        conversationId,
        message,
        assistContext: assistContext as OrchestratorAssistContext,
        preferredLanguage,
        req,
